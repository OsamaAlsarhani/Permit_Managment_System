...
public class UserPermitController {
    ...
    public UserPermitController(PermitService permitService, MinioService minioService) {
        this.permitService = permitService;
        this.minioService =minioService;
    }

    // Handle JSON only (no files)
    @PostMapping(value = "/create-permit")
    public ResponseEntity<?> createPermit(@RequestBody PermitDTO permitDTO,@RequestParam(required = false)  List<String> filesURL) {
        // Call your service with no files
        return ResponseEntity.ok(permitService.createPermit(permitDTO, filesURL));
    }
    ...
    public ResponseEntity<List<String>> uploadFiles(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Collections.singletonList("File upload failed: " + e.getMessage()));
        }
    }
