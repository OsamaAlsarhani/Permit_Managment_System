...
public class PermitService {
    ...
    public PermitService(PermitRepo permitRepo, UserRepo userRepo, FileStorageService fileStorageService, MinioService minioService) {
    ...
    public PermitDTO createPermit(PermitDTO permitDTO, List<String> filesURL) {
        ...
        // Combine URLs from DTO and uploaded files

        permit.setFileUrls(filesURL);

        permit.setApplicant(applicant);
        permit.setStatus(PermitStatus.PENDING);
        permit.setAdminComment(null);
    ...
    public String uploadFileToPermit(Integer permitId, MultipartFile file) {
        ...
        if (permit.getApplicant() == null || !permit.getApplicant().getUsername().equals(username)) {
            throw new AccessDeniedException("You do not have permission to upload to this permit.");
        }

        String fileName = fileStorageService.storeFile(file);

        if (permit.getFileUrls() == null) {
            permit.setFileUrls(new ArrayList<>());
        }
