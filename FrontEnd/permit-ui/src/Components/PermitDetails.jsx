...
const DetailPermit = ({ permit, onClose }) => {
  ...
  const statusStyle =
    statusColors[permit.status] || "bg-gray-100 text-gray-800";
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] p-6 overflow-y-auto relative animate-fadeIn">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <p>
              <strong>Applicant Name:</strong> {permit.applicantName}
            </p>
            <p>
            ...
            <p>
              ...
              <span
                className={`inline-block px-2 py-1 rounded text-sm font-semibold ${statusStyle}`}
              >
                {permit.status}
              </span>
            </p>
            <p>
              <strong>National ID / CR:</strong> {permit.nationalIdOrCr}
            </p>
            <p>
              <strong>Location:</strong> {permit.location}
            </p>
            <p>
              <strong>Area:</strong> {permit.area} m²
            </p>
            <p>
              <strong>Contact:</strong> {permit.contactNumber}
            </p>
            <p>
              <strong>Email:</strong> {permit.email}
            </p>
            <p>
              <strong>Start Date:</strong> {formatDate(permit.startDate)}
            </p>
            <p>
              <strong>End Date:</strong> {formatDate(permit.endDate)}
            </p>
          </div>
          ...
          {permit.fileUrls.map((url, idx) => {
            ...
            return (
              <li key={idx}>
                <Link
                  to={`/view-file/${encodeURIComponent(url)}`}
